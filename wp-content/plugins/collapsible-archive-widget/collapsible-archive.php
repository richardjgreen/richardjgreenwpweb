...
EOB;
		...
		$string_to_echo .= "<span id=\"ara_ca_mosign{$years[$x]->year}\">$icon</span></a><a href=\"$year_link\" style=\"position:absolute;\">{$years[$x]->year}</a>";
		if($count > 0) $string_to_echo .= '&nbsp;('.$years[$x]->posts.')';
		$string_to_echo .= $childOpen.' id="ara_ca_mo'.$years[$x]->year.'" style="margin-left:20px; display:'.(($defaultexpand || ($expandcurryear && date("Y") == $years[$x]->year)) ? 'block' : 'none').'">';
...
EOB;
			...
			{
				$add_before = <<<EOB
					<a style="cursor:pointer;margin-right:25px;" onclick="collapsiblearchive_toggle('ara_ca_po$year{$month->month}','ara_ca_posign$year{$month->month}')">
EOB;
			}
			$add_before .= "<span id=\"ara_ca_posign$year{$month->month}\" style=\"position:absolute;\">$icon</span></a>";
		}
		else $add_before = '';
...
function widget_ara_collapsiblearchive_init() {
	...
	/*
	Original Code Block for Image Paths
	Retained in the Event that I Wish to Revert

	$ara_collapsible_icons['plus'][0] = '<img src="'http://themecdn.richardjgreen.net/images/wp7plus-black.png" height="20" width="20" alt="Expand Arrow" />&nbsp;';
	$ara_collapsible_icons['minus'][0] = '<img src="'http://themecdn.richardjgreen.net/images/wp7minus-black.png" height="20" width="20" alt="Collapse Arrow" />&nbsp;';
	*/	
